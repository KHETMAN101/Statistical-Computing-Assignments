...
```{r}
mean_dist_tbl <- flights |> # for mean calculation
  filter(month == 1) |>
  group_by(carrier) |>
  summarise(mean_distance = 
              mean(distance, na.rm = TRUE)) 

sd_dist_tbl <- flights |> # for sd calculation
  filter(month == 1) |>
  group_by(carrier) |>
  summarise(sd_distance = 
              sd(distance, na.rm = TRUE)) 

dist_tbl <- mean_dist_tbl |> # To calculate dist_tbl
  left_join(sd_dist_tbl, by ='carrier') |>
...
```{r}
dataset_Q7 <-structure(list(id = c("id_1", "id_2", "id_3", "id_4", "id_5", 
